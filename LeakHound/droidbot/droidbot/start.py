...
def parse_args():
    ...
    parser.add_argument("-interval", action="store", dest="interval", default=1.0,
                        type=float,
                        help="Interval in seconds between each two events. Default: %f" % 1.0)
    ...
    parser.add_argument("-replay_output", action="store", dest="replay_output",
                        help="The droidbot output directory being replayed HEY HEY HEY.")
    parser.add_argument("-frida_scripts", action="store", dest="frida_scripts",
                        help="Path to the frida scripts || MODIFIED.")
    parser.add_argument("-frida_output_folder", action="store", dest="frida_output_folder",
                        help="Path to the frida output folder || MODIFIED.")
    parser.add_argument("-package_name", action="store", dest="package_name",
                        help="The name of the app's package || MODIFIED. ############")
...
def main():
    ...
    """
    print("TESTAR O PRINT NO DROIDBOT")
    opts = parse_args()
    import os
    ...
    if start_mode == "master":
        ...
        droidbot = DroidBot(
            ...
            ignore_ad=opts.ignore_ad,
            replay_output=opts.replay_output,
            frida_output_folder=opts.frida_output_folder,
            frida_script=opts.frida_scripts,
