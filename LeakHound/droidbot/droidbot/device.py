...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
                 ...
                 frida_scripts: str = None, frida_output_folder: str = None,
                 package_name: str = None, device: str = None):
        """
        initialize a device connection
        :param device_serial: serial number of target device
        :param is_emulator: boolean, type of device, True for emulator, False for real device
        :param frida_scripts: str, Path to the frida scripts to hook, if any || MODIFIED
        :param frida_output_folder: str, The path to the folder where all the output will be written || MODIFIED
        :param package_name: str, The name of the app's package || MODIFIED
        :return:
        """

        ### MODIFICATION ###
        self.frida_script = frida_scripts
        self.frida_output_folder = frida_output_folder
    ...
    def send_intent(self, intent):
        ...
        try:
            if "am start" in cmd:
                print(f"DETECTED A START COMMAND: {cmd}")
                if self.frida_script:
                    print("Overwriting with frida injection")
                    print("About to hook scripts")
                                ...
                                if "payload" in message:
                                    ...
                                    try:
                                        ...
                                        if msg_content is not None:
                                            output_path = os.path.join(self.frida_output_folder, f"{script_name}.txt")
                                        ...
                                        output_path = os.path.join(self.frida_output_folder, "common.txt")
                                        with open(output_path, "a", encoding="utf-8") as frida_file:
                    ...
                    # Spawn the app (does NOT start execution yet)
                    ...
                    script_files = glob.glob(os.path.join(self.frida_script, "**", "*.js"), recursive=True)
                    ...
                    for file_path in script_files:
                        try:
                            ...
                            print(f"Failed to open file {file_path} because: {e}")

                    # Resume the app (equivalent to --no-pause)
                    print("Resuming")
                    device.resume(pid)

                else:
                    return self.adb.shell(cmd)
            ####### END OF MOFDIFICATION ###########
        except Exception as e:
            print(f"FRIDA MODIFICATION BROKE: {e}")
                    ...
                    try:
                        if "payload" in message:
                            ...
                            try:
                                ...
                                output_path = os.path.join(self.frida_output_folder, "common.txt")
                                with open(output_path, "a", encoding="utf-8") as frida_file:
                                    json.dump({"raw_payload": payload}, frida_file)
                                    frida_file.write("\n")
                        else:
                            print(f"Payload missing in message: {message}")
                            pass
